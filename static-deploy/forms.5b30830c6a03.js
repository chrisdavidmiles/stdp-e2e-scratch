 ...
 *
 * Intercepts submission of any form patched with `data-static-deploy-patched="..."` and POSTs the
...
(function () {
	...
	function findFormSlug(form) {
		// Prefer `data-static-deploy-slug`, fall back to id, fall back to form name attribute.
		return form.getAttribute('data-static-deploy-slug') || form.id || form.getAttribute('name') || '';
	}

	function findFormId(form) {
		return form.getAttribute('data-static-deploy-form-id') || form.id || '';
	}
	...
	function envelope(form) {
		...
		return {
			...
			meta: {
				...
				form_provider: form.getAttribute('data-static-deploy-patched') || provider
			}
		};
	}
	...
	function intercept(event) {
		...
		if (!form || !form.matches || !form.matches('form[data-static-deploy-patched]')) {
			return;
		}
		event.preventDefault();
		var data = envelope(form);
		if (shouldReject(data.fields)) {
			form.dispatchEvent(new CustomEvent('static-deploy:form:rejected', { bubbles: true, detail: { reason: 'honeypot' } }));
			return;
		}
		fetch(endpoint, {
			method: method,
			...
			.then(function (resp) {
				if (resp.ok) {
					form.dispatchEvent(new CustomEvent('static-deploy:form:success', { bubbles: true, detail: { status: resp.status } }));
					if (form.reset) form.reset();
				} else {
					form.dispatchEvent(new CustomEvent('static-deploy:form:error', { bubbles: true, detail: { status: resp.status } }));
				}
			})
			.catch(function (err) {
				form.dispatchEvent(new CustomEvent('static-deploy:form:error', { bubbles: true, detail: { error: String(err) } }));
			});
	}
